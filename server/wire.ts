...
/**
 * MVU 状态栏属于当前楼层的显示投影，不属于每条历史消息的永久 UI。
 * 保留消息里的占位协议，但旧楼层在皮肤替换前去掉，避免创建一堆状态栏 iframe。
 */
function projectStatusPlaceholder(text: string, skin: DisplaySkin | null, show: boolean): string {
	if (!skin?.rules.some((r) => STATUS_PLACEHOLDER_RE.test(r.source))) return text;
	const placeholder = /<StatusPlaceHolderImpl\s*\/?>/gi;
	if (!show) return text.replace(placeholder, "");
	...
	if (msg.role === "assistant") {
		...
		const timeline =
			Array.isArray(rpTimeline) && rpTimeline.length > 0
				? (() => {
						const source = rpTimeline as WireSegment[];
						let lastText = -1;
						for (let i = source.length - 1; i >= 0; i--) {
							if (source[i].kind === "text") {
								lastText = i;
								break;
							}
						}
...
export function toWireHistory(
	...
	let backstage = false;
	const skin = opts?.skin ?? null;
	// 只让当前分支最后一条可见剧情角色消息拥有状态栏；没有角色回复时由开场白承载。
	let latestStatusIndex = -1;
	for (let i = messages.length - 1; i >= 0; i--) {
		const candidate = messages[i] as MsgLike | null;
		if (!candidate || typeof candidate !== "object") continue;
		if (candidate.role === "assistant" && textOf(candidate.content).trim() && !hasToolCall(candidate.content)) {
			latestStatusIndex = i;
			break;
		}
	}
	if (latestStatusIndex < 0) {
		for (let i = messages.length - 1; i >= 0; i--) {
			const candidate = messages[i] as MsgLike | null;
			if (candidate?.role === "custom" && candidate.customType === "rp-greeting" && textOf(candidate.content).trim()) {
				latestStatusIndex = i;
				break;
			}
		}
