...
export default function App() {
	...
	// 作者状态栏只读取 Agent 投影的 MVU 树，不依赖酒馆变量系统。
	useEffect(() => {
		...
		const onReady = (event: MessageEvent) => {
			const data = event.data as { liyuanVariablesReady?: unknown } | null;
			if (data && typeof data === "object" && "liyuanVariablesReady" in data) {
				(event.source as Window | null)?.postMessage(payload, "*");
			}
		};
