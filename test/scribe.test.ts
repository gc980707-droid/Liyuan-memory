...
test("场记提示词：要求推进时间并保护历史档案", () => {
	const { systemPrompt } = buildScribeTurnPrompt({
		state: { ...defaultState(), mvu: { 坐标: { 时间: "2026-06-01 14:30" } } },
		userText: "我检查了现场。",
		assistantText: "你检查了现场。",
		charName: "青梧",
		userName: "阿远",
	});
	assert.ok(systemPrompt.includes("至少推进 5 分钟"));
	assert.ok(systemPrompt.includes("历史档案字段"));
});
