...
test("引擎循环：已有稿后普通文本续写不丢失", async () => {
	const { cwd, sm } = makeStage();
	const reg = registerFauxProvider({ models: [{ id: "faux-rp" }] });
	try {
		reg.setResponses([
			fauxAssistantMessage("第一段正文。"),
			fauxAssistantMessage("第二段正文。"),
			fauxAssistantMessage(""), // 续稿后的封笔催告
			fauxAssistantMessage(""), // 记账席位
			fauxScribeEmpty(),
		]);
		const engine = makeEngine(cwd, sm, reg.getModel("faux-rp"));
		...
		const { history } = rebuildHistory(sm.getBranch() as BranchEntryLike[]);
		assert.equal(history[history.length - 1].text, "第一段正文。\n\n第二段正文。", "后续普通文本应并入现稿");
	} finally {
		reg.unregister();
		rmSync(cwd, { recursive: true, force: true });
	}
...
test("引擎记账：MVU 会话不丢弃 world_state_update，并同步时间与背包", async () => {
	const { cwd, sm } = makeStage();
	const reg = registerFauxProvider({ models: [{ id: "faux-rp" }] });
	try {
		const initial = {
			坐标: { 时间: "2026-06-01 14:30", 当前状态: "普通-零号大坝", 当前位置: { 区域: "游客中心", 具体设施: "二楼休息室" } },
			主角: { 资产: { 背包内容: { 存放物品: {} } } },
		};
		sm.appendCustomEntry("rp-state", { ...defaultState(), mvu: initial });
		reg.setResponses([
			fauxAssistantMessage([fauxToolCall("world_state_update", { patch: { inventory: ["水手哨"] } }), fauxToolCall("draft_write", { content: "我收起水手哨。" })], { stopReason: "toolUse" }),
			fauxAssistantMessage(""),
		]);
		const engine = makeEngine(cwd, sm, reg.getModel("faux-rp"));
		await engine.performTurn("我收起水手哨。");
		const state = stateFromBranch(sm.getBranch() as BranchEntryLike[]);
		assert.equal(state.mvu?.坐标 && (state.mvu.坐标 as Record<string, unknown>).时间, "2026-06-01 14:35");
	} finally {
		reg.unregister();
		rmSync(cwd, { recursive: true, force: true });
	}
