...
test("补丁：status_fields 多角色合并与删除（卡状态栏字段）", () => {
	let st = applyPatch(defaultState(), {
		status_fields: { 苏小棉: { "👤 姓名": "苏小棉", "📝 当前行动": "发推文" } },
	}).state;
	assert.equal(st.status_fields?.["苏小棉"]?.["👤 姓名"], "苏小棉");
	st = applyPatch(st, {
		status_fields: { 苏小棉: { "📝 当前行动": "假装伸懒腰", "👗 当前穿搭": "水手服" }, 旅人: { "👤 姓名": "旅人" } },
	}).state;
	assert.equal(st.status_fields?.["苏小棉"]?.["📝 当前行动"], "假装伸懒腰", "按角色按键合并");
	assert.equal(st.status_fields?.["苏小棉"]?.["👤 姓名"], "苏小棉", "未变字段保留");
	assert.equal(st.status_fields?.["旅人"]?.["👤 姓名"], "旅人", "多角色分组");
	const r = applyPatch(st, { status_fields: { 苏小棉: { "👤 姓名": null } } });
	assert.equal(r.state.status_fields?.["苏小棉"]?.["👤 姓名"], undefined, "角色内字段 null 删除");
	assert.equal(r.warnings.length, 0);
});

test("补丁：status_fields 扁平旧形态并入主角", () => {
	const st = applyPatch(defaultState(), { status_fields: { "👤 姓名": "苏小棉" } }).state;
	const buckets = st.status_fields ?? {};
	const flat = Object.values(buckets).find((b) => b["👤 姓名"] === "苏小棉");
	assert.ok(flat, "扁平值并入某角色桶");
});
