...
test("toWireHistory：删除最新楼层后上一楼状态栏恢复显示", () => {
	const skin = {
		rules: [{ name: "status", source: "<inner>[\\s\\S]*?</inner>", flags: "g", replace: "<div>状态栏</div>" }],
		charName: "卡",
		userName: "旅人",
	};
	const history = [
		{ role: "assistant", content: [{ type: "text", text: "第一楼\n<inner>旧状态</inner>" }] },
		{ role: "user", content: "第二楼" },
