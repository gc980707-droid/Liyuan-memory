...
import { parseGreetingInitvar } from "../../src/mvu.ts";
...
export default function roleplayExtension(pi: ExtensionAPI) {
	...
	pi.on("session_start", async (event, ctx) => {
		...
		try {
			...
			// 新会话优先使用当前选中开场白自己的 Initvar；已有状态不重置。
			if (!state.mvu) {
				const greeting = buildGreeting(card, config);
				const mvu = parseGreetingInitvar(greeting);
				if (mvu) {
					state = projectMvuToWorldState({ ...state, mvu });
