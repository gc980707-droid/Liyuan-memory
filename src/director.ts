...
${
	statusBarFormats && statusBarFormats.length > 0
		? `   - **本卡定义了状态栏显示能力**（格式线索仅用于兼容旧卡）：${statusBarFormats.join("；")}。
   - 不要在正文中输出状态栏，也不要依赖角色卡正则。Harness 会在本轮记账后根据 World State/MVU 自动生成只读状态面板。`
		: `   - 本卡未检测到专用状态栏格式；Harness 仍可按 World State/MVU 提供通用状态面板，不要把状态栏写入正文。
...
export function buildTurnInjection({
	...
	if (memoryIndex) {
		blocks.push(
			`【记忆目录】以下是当前对话记忆库的分层常驻索引。分段总览覆盖全部历史，本轮相关条目从全库选出；它只证明“有这些记忆”，不是完整事实。看到相关条目时结合下方【剧情记忆】片段，不要因当前正文没提到就当作不存在。记忆是旧资料，不能覆盖当前世界状态、当前 MVU、最新摘要或本轮正文：\n${memoryIndex}`,
		);
	}
	...
	if (memoryHits && memoryHits.length > 0) {
		...
		blocks.push(
			`【剧情记忆】以下为与本轮相关的向量记忆片段（可能含旧摘要或外部资料），按需取用，勿整段照抄堆砌。它们不是当前状态，也不能推翻最新摘要、世界账本、MVU 或本轮正文；发生冲突时以更新的来源为准：\n${lines.join("\n")}`,
		);
	}
