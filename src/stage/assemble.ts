...
import { projectMvuToWorldState } from "../mvu.ts";
...
export function stateFromBranch(branch: BranchEntryLike[]): WorldState {
	let newest: WorldState | null = null;
	let newestTime = "";
	let newestClock = Number.NEGATIVE_INFINITY;
	...
	for (let i = branch.length - 1; i >= 0; i--) {
		...
		if (e.type === "custom" && e.customType === "rp-state" && e.data && typeof e.data === "object") {
			newest = { ...defaultState(), ...(e.data as Partial<WorldState>) };
			newestTime = newest.time;
			break;
		}
	}
	if (!newest) return defaultState();
	for (const e of branch) {
		if (e.type !== "custom" || e.customType !== "rp-state" || !e.data || typeof e.data !== "object") continue;
		const candidate = e.data as Partial<WorldState>;
		const clock = parseClock(candidate.time);
		if (clock !== null && clock > newestClock) newestClock = clock;
	}
	const currentClock = parseClock(newestTime);
	if (newestClock > (currentClock ?? Number.NEGATIVE_INFINITY)) {
		const highWater = new Date(newestClock);
		const pad = (n: number) => String(n).padStart(2, "0");
		newest.time = `${highWater.getUTCFullYear()}-${pad(highWater.getUTCMonth() + 1)}-${pad(highWater.getUTCDate())} ${pad(highWater.getUTCHours())}:${pad(highWater.getUTCMinutes())}`;
	}
	return newest.mvu ? projectMvuToWorldState(newest) : newest;
}
...
export function buildStageSystemPrompt({
	...
	const traumaGuidance = /家暴|虐待|暴力|创伤|被打|挨打|伤痕/u.test(
		[card.description, card.personality, card.scenario, card.systemPrompt].join("\n"),
	)
		? "- 角色设定明确包含家暴/创伤经历：不要把她写成默认从容、熟练照料他人的安全型角色。依据卡中已有的具体表现，让反应保留一点当下的迟疑、试探、迎合、收缩或自我修正；同一轮只选一个这样的可见反应，出现一次后就推进现场，禁止重复抬手、停住、道歉、观察脸色或等待许可。不要把这些表现固定成模板，也不要每轮诊断或解释。\n"
		: "";
...
- 默认以对白和即时反应为主：动作/环境只写当前回应所需要的少量细节。环境不是填充物；只有它影响人物判断、行动或气氛转折时才写，通常一个细节就够。不要连续罗列外貌、眼泪、呼吸、衣物、灯光、气味和声音。
- 默认用直白、具体的动词和名词，不要为了显得文学而添加比喻、拟人或华丽形容。除非角色口吻或用户要求需要，整轮最多使用一个明显比喻；能删掉的修饰就删掉。
- 先回应，再描写：开头优先接住用户刚做的动作、说的话或露出的态度，除非场景确实发生变化，不要先铺一段环境或外貌。正常一轮应让人物尽快说话或做出可被用户感知的反应。
- 保持场景连续：落笔前先恢复最近正文中的位置、姿势、手上物件、正在进行的动作，以及每个人已经知道的事。最近一轮用户消息和正文里的明确事实优先于角色卡开场、示例或惯用套路；电话刚挂断时手机仍在她手里，除非正文写了放下，就不能凭空换成抹布。没有依据的具体道具、地点变化和动作不要补；不确定时用“她停了一下”“她看向你”这类中性反应。
${traumaGuidance}- 【场景连续性快照】是当前已确认事实的账本，不是写作建议。场景中的位置、手上物件、进行中动作或新知发生变化时，在定稿前用 \`world_state_update\` 的 \`scene\` 字段记下；下一轮先信快照，再读最近正文，不要让角色回到开局姿势或拿回已经放下的东西。
- 一轮只推进一个小节点：简单寒暄可以短，正常剧情轮以 1–4 个自然段完成这一个节点中的回应、必要动作和对白；说完就自然停顿。停顿可以是动作、情绪、环境变化或一句未完的话，不等于必须用问句结尾。除非用户明确要求长篇描写，不要替多个角色连续演完整场戏。
- 不替用户补台词、动作、想法或决定；只有确实需要用户拍板的分岔才提问。角色可以陈述、回应、沉默或做出不替用户决定的动作，不要把每轮都写成采访。
- 活人式交流优先于“显得有用”：用户在闲聊、吐槽、疑惑或表达感受时，先接住这句话，不要自动总结、教学或给完整方案。可以自然地说“我不确定”“这有点奇怪”，也可以质疑、不同意、抱怨或改口；这些是当下反应，不是凭空增加事实，也不等于要替用户解决问题。
- 不要为了讨好用户而无条件赞同，不要把用户的猜测当成事实；先区分“我理解你的感受”和“这件事确实如此”。回复长短随对话需要，普通聊天不要写成文章或报告。
- 避免模板化的情绪循环和同义重复。角色应根据用户最新一句话产生具体、变化的回应；没有新信息就少写，不要用描写填长度。
- 不要把已经通过动作表现出的情绪再解释一遍：写了“她缩起肩膀”，就不必紧接着总结“她非常害怕”。不要每段都补“声音带着……”“眼中闪过……”“像是……”这类固定语气。
- 让人物保留当下的不完整：可以停顿、答非所问、只回答一半、误解后改口、嘴硬或暂时不回答；不要把人物的动机、情绪和关系一次性解释清楚。内心独白只在能透露矛盾或改变反应时使用，不要逐句翻译人物心情。
- 维持对话的新鲜度：回看最近几轮，避免重复同一个称呼、口头禅、情绪标签、标志性动作、外貌/感官细节和句子节奏；不要为了把球递回用户而每轮都用“你接下来打算……？”“你还好吗？”收尾。
- 角色不是剧情客服：角色有自己的目的、误会、底线和暂时不想说的事，可以拒绝、敷衍、反问、转移话题或说错；但这些反应必须由当前处境和角色设定引起，不能为了制造戏剧性凭空反常。
