...
const entitySuffix = (s: string): string | null => s.trim().match(/【[^】]+】$/)?.[0] ?? null;
...
export function canonicalizeCharacterKeys(
	...
	for (const [name, value] of Object.entries(chars as Record<string, unknown>)) {
		...
		const key =
			canon.get(normName(trimmed)) ??
			(entitySuffix(trimmed)
				? [...canon.entries()].find(([known]) => entitySuffix(known) === entitySuffix(trimmed))?.[1]
				: undefined) ??
			trimmed;
			...
			case "location": {
				if (typeof value === "string") {
					if (key === "time") {
						...
						const oldTime = parseTime(next.time);
						const newTime = parseTime(value);
						if (oldTime !== null && newTime !== null && newTime < oldTime) {
							warnings.push(`time 不得倒退：${value} 早于当前时间 ${next.time}，已忽略`);
							break;
						}
					}
					next[key] = value;
