...
export function buildScribeTurnPrompt(input: ScribePromptInput): { systemPrompt: string; userText: string } {
	...
	const hasMvu = !!state.mvu && typeof state.mvu === "object";
	...
	const mvuInstruction = hasMvu
		? `

【作者卡 MVU 状态树】
本账本还包含作者卡自己的 MVU 状态树。请同时检查本轮剧情是否改变了其中的事实，尤其是时间、地点、主角装备/物品、在场角色和角色档案。每个完成的剧情回合默认至少推进 5 分钟；若正文明确给出经过时间，按正文计算并优先更新坐标.时间。把变化写入 "mvu_patch"，键使用状态树中的点路径，值使用变化后的完整值；没有变化时为 {}。不要为了凑更新修改没有发生变化的字段。历史档案字段（过往、初见、死因、外貌特征、名器特点、性交经历）只读，禁止用本轮推断覆盖。`
		: "";
	const rulesSection = input.mvuRules?.trim() ? `\n\n【作者卡 MVU 更新规则】\n${input.mvuRules.trim()}` : "";
...
【用户动作边界】只有用户原话明确写出的动作才算用户已执行。主演正文中“你开枪/你扣扳机/你移动”等，如果用户原话没有明确写出，只能视为错误续写，不得据此扣弹药、改位置或记为用户行为。${hasMvu ? `

【MVU 状态树当前值】
${JSON.stringify(state.mvu, null, 2)}` : ""}`;
