...
- **「🐦 推特日记」**：正文写到了推文就按正文；正文没写新推文时，根据本拍剧情虚构一条符合角色人设与剧情氛围的推文（营业口吻、带 #标签），配图从【卡图池】选一张合适的。**输出格式严格按下面这个结构（用 | 分隔）**：
  ⏱️ 时间：<时间> | 推文：<推文正文> | 📸 [IMG:url|描述] | 💗 点赞x | 转发x | 浏览x | 💬 <用户名>：<评论> / <用户名>：<评论>
  （💬 评论 2~3 条，抽象乐子人/直发情哥/理中客风格；图池没有合适的就不写 📸 段；输出中不要出现换行，各段用 | 连接）
